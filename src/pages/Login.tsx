...
export default function Login() {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    if (!validation.success) {
      setError(validation.error.issues.map((e: { message: string }) => e.message).join(', '));
      return;
    }
      ...
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-900 rounded-2xl p-8 shadow-xl border border-gray-100 dark:border-gray-800" noValidate>
          ...
          <div className="space-y-5">
            ...
            <button type="submit" disabled={loading} className="btn-primary w-full flex items-center justify-center gap-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-eco-500 focus-visible:ring-offset-2">
          ...
          <p className="text-center text-sm text-gray-600 dark:text-gray-400 mt-6">
            ...
            <Link to="/signup" className="text-eco-600 dark:text-eco-400 font-medium hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-eco-500 rounded">
              Sign Up
            </Link>
          </p>
        </form>
